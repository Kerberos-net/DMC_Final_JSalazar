...
/// "SmartNet.Sugerencia.Core: net10.0, zero PackageReference".
///
/// Copy of SmartNet.Contable.Core.Tests/PurityScanTests.cs (item #9's own design.md Decision 2:
/// the orchestrator lives in Sugerencia.Core too, so this scan covers it as well), retargeted at
/// SmartNet.Sugerencia.Core.dll. The one ProjectReference (Catalogos.Core) is itself pure, so this
/// scan stays meaningful transitively. tasks.md Phase 6.
/// </summary>
public class PurityScanTests
{
    private static readonly string CoreAssemblyPath = Path.Combine(
        AppContext.BaseDirectory, "SmartNet.Sugerencia.Core.dll");
    ...
    public void DomainCore_DoesNotReferenceSystemDataSqlClient()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Sugerencia.Core must not reference System.Data.SqlClient (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotReferenceMicrosoftDataSqlClient()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Sugerencia.Core must not reference Microsoft.Data.SqlClient (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotReferenceMicrosoftAspNetCore()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Sugerencia.Core must not reference any Microsoft.AspNetCore.* type (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotReferenceSystemNetHttp()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Sugerencia.Core must not reference System.Net.Http (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotCallDateTimeNowOrUtcNowDirectly()
    {
        ...
        Assert.True(offendingCallSites.Count == 0,
            "SmartNet.Sugerencia.Core must not call DateTime.Now/DateTime.UtcNow directly — " +
            "time MUST be received as a parameter, per ADR 0019 / spec.md. " +
            "Offending call sites: " + string.Join(", ", offendingCallSites));
    }

    [Fact]
    public void CascadaDeSugerencia_ExposesNoTaskReturningMember()
    {
        // tasks.md 6.2: CascadaDeSugerencia is the pure cascade (spec.md "MUST be pure") — unlike
        // ServicioDeSugerencia, which legitimately awaits the 4 async ports, this static class
        // must expose zero Task-returning members.
        var offendingMembers = typeof(CascadaDeSugerencia)
        ...
        Assert.True(offendingMembers.Count == 0,
            "CascadaDeSugerencia declares Task-returning member(s): " +
