...
/// <summary>
/// spec.md "Pure promotion decision" (factura-promotion) / ADR 0019 ("núcleo sin dependencias de
/// base de datos, HTTP ni reloj") / design.md "SmartNet.Inbox.Core: zero PackageReference".
///
/// Copy of SmartNet.TiposCambio.Core.Tests/PurityScanTests.cs (item #7 tasks.md 2.2), retargeted
/// at SmartNet.Inbox.Core.dll.
/// </summary>
public class PurityScanTests
{
    private static readonly string CoreAssemblyPath = Path.Combine(
        AppContext.BaseDirectory, "SmartNet.Inbox.Core.dll");
    ...
    public void DomainCore_DoesNotReferenceSystemDataSqlClient()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Inbox.Core must not reference System.Data.SqlClient (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotReferenceMicrosoftDataSqlClient()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Inbox.Core must not reference Microsoft.Data.SqlClient (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotReferenceMicrosoftAspNetCore()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Inbox.Core must not reference any Microsoft.AspNetCore.* type (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotReferenceSystemNetHttp()
    {
        ...
        Assert.True(result.IsSuccessful,
            "SmartNet.Inbox.Core must not reference System.Net.Http (ADR 0019 / spec.md).");
    }
    ...
    public void DomainCore_DoesNotCallDateTimeNowOrUtcNowDirectly()
    {
        ...
        Assert.True(offendingCallSites.Count == 0,
            "SmartNet.Inbox.Core must not call DateTime.Now/DateTime.UtcNow directly — time MUST " +
            "be received as a parameter, per ADR 0019 / spec.md. Offending call sites: " +
