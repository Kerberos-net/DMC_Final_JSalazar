# Exporta los catalogos maestros desde los .xlsx de "Documentacion del negocio"
...
$docsNeg = Join-Path $repo 'Documentación del negocio'
...
Write-Csv 'DocumentoIdentidad' @('A','B') (ConvertFrom-XlsxSheet (Join-Path $docsNeg 'DocumentoIdentidad.xlsx'))

# --- Origen: codigo | origen
Write-Csv 'Origen' @('A','B') (ConvertFrom-XlsxSheet (Join-Path $docsNeg 'Origen.xlsx'))

# --- Motivo: codigo | motivo | cuenta (prefijos separados por coma)
Write-Csv 'Motivo' @('A','B','C') (ConvertFrom-XlsxSheet (Join-Path $docsNeg 'Motivos.xlsx'))

# --- CuentaContable: cuenta | descripcion | nivel | ctarefleja | ctapuente
Write-Csv 'CuentaContable' @('A','B','C','D','E') (ConvertFrom-XlsxSheet (Join-Path $docsNeg 'Cuentas.xlsx'))

# --- Proveedor: codpro | proveedor | coddocide | rucpro  (con el prefijo DNI retirado)
$proveedores = ConvertFrom-XlsxSheet (Join-Path $docsNeg 'Proveedores.xlsx')
$conPrefijo = 0
