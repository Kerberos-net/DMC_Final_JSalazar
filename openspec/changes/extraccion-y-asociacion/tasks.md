...
- [ ] 2.1 Create `SmartNet/worker/tests/fixtures/comprobante_con_texto.pdf`, `comprobante_escaneado.pdf` — two synthetic PDFs (one with an embedded text layer, one scanned/image-only).
- [ ] 2.2 RED: `tests/unit/test_pdf_texto.py` — RUC next to `R.U.C.` with/without dots; two RUCs present resolved against `EMPRESA.RUC` config; serie-número with/without spaces; SUNAT filename fallback `<RUC>-<TIPO>-<SERIE>-<NUMERO>.pdf`; **partial filename fallback → no key** (all-or-nothing).
- [ ] 2.3 Confirm RED: `pytest tests/unit/test_pdf_texto.py -q` fails on collection.
- [ ] 2.4 GREEN: create `SmartNet/worker/src/smartnet_worker/pdf_texto.py` — `ExtraccionPdf` dataclass, `extraer(texto, nombre_archivo) -> ExtraccionPdf`, regex extraction + SUNAT filename backup.
- [ ] 2.5 Confirm GREEN: `pytest tests/unit/test_pdf_texto.py -q` passes.
- [ ] 2.6 RED: `tests/unit/test_pdf_lectura.py` — page with a text layer → `MotorOcr.reconocer` never invoked (fake records 0 calls); page with no text layer → OCR of exactly that page at 300 DPI; encrypted PDF and corrupt PDF → `PdfIlegibleError` (never a crash); `_MAXIMO_PAGINAS_OCR = 5` cap respected.
- [ ] 2.7 Confirm RED: `pytest tests/unit/test_pdf_lectura.py -q` fails on collection.
- [ ] 2.8 GREEN: create `SmartNet/worker/src/smartnet_worker/pdf_lectura.py` — `LectorPdf`/`MotorOcr` protocols, `LectorPdfLocal` (pypdf text layer + pypdfium2 rasterization), `MotorTesseract` (pytesseract + `spa`), `verificar_tesseract()` preflight, `PdfIlegibleError`, `TesseractNotFoundError`.
- [ ] 2.9 Modify `SmartNet/worker/src/smartnet_worker/config.py` — add `TESSERACT_CMD_ENV_VAR` (`SMARTNET_WORKER_TESSERACT_CMD`, optional), `OCR_IDIOMA='spa'`, `OCR_DPI=300`, `obtener_tesseract_cmd() -> str | None` (absence is legal, unlike credential/storage-root vars).
- [ ] 2.10 Confirm GREEN: `pytest tests/unit/test_pdf_lectura.py -q` passes; `ruff check src/ tests/` clean; full non-integration unit suite has no regression against items #4/#5.

### Work Unit Evidence (WU2)

| Evidence | Value |
|---|---|
| Focused test command | `pytest tests/unit/test_pdf_texto.py tests/unit/test_pdf_lectura.py -q` |
| Runtime harness | N/A — MotorOcrFalso keeps the unit suite Tesseract-free; real OCR is only exercised by WU4's `ocr` marker |
| Rollback boundary | Delete `pdf_texto.py`, `pdf_lectura.py`, their tests, the two new PDF fixtures; revert `config.py` diff |
