...
|---|---|---|---|---|
| 1 | `RucProveedor` (`Factura`, `DatosExtraidos`) | `CHAR(11) NULL` + `CHECK (… LIKE '[0-9]…[0-9]')` | Peruvian RUC is **exactly** 11 digits and is an **identifier**: leading zeros are data and no arithmetic is ever performed on it. `CHAR` over `VARCHAR` makes a wrong-length value impossible to store silently. Nullable because "campos no extraídos" is an explicitly supported state. **Not an FK to `dbo.Proveedor`** — it is a frozen copy | Cannot happen; 11 is a legal fixed length |
