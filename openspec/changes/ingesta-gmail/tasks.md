...
- [ ] 2.1 Modify `SmartNet/worker/pyproject.toml` — add `google-api-python-client>=2.140`, `google-auth>=2.34`; register `smartnet-gmail` script entry point.
- [ ] 2.2 Modify `SmartNet/worker/src/smartnet_worker/config.py` — add `GMAIL_CREDENTIALS_ENV_VAR` (`SMARTNET_WORKER_GMAIL_CREDENTIALS`), `STORAGE_ROOT_ENV_VAR` (`SMARTNET_WORKER_STORAGE_ROOT`), `GMAIL_SCOPES=["https://www.googleapis.com/auth/gmail.modify"]`, and two getters raising `ConfiguracionError` on missing/malformed value (no default in code).
- [ ] 2.3 Create `SmartNet/worker/src/smartnet_worker/gmail_client.py` — `ClienteGmail` with `resolver_etiquetas`, `buscar_mensajes` (paginated), `obtener_mensaje`, `obtener_adjunto`, `aplicar_etiqueta`; one method per API call, zero branching/parsing (design Decision 1 & 3 — missing label fails loudly before search).
- [ ] 2.4 Create `SmartNet/worker/src/smartnet_worker/almacenamiento.py` — `escribir(raiz, ruta_relativa, datos)` with `resolved.is_relative_to(raiz)` containment guard (defense in depth beyond `gmail.py`'s sanitizer).
- [ ] 2.5 Smoke check: both new modules import cleanly and `ruff check src/` passes; no unit test file is added here per design's Testing Strategy (both are exercised through WU4's fakes/`tmp_path`) — compression acknowledged.
