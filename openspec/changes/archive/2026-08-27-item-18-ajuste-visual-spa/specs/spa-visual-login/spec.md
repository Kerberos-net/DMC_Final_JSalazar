# Delta for spa-visual-login

Revises archived `2026-08-24-diseno-visual-spa-item-12`. Conforms `login-page` structure to the ratified handoff without touching authentication logic.
...
(Previously: used the shared validation-error token but via the `.banner--error` block.)
...
(Previously: same rule, before radius/elevation tokens existed.)
