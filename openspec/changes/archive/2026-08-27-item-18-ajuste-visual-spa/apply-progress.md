...
- PR8 / `pr8/item-18-proveedor-picker` off `pr6/item-18-asientorespuesta-base-igv-projection` (tip `37a4ece`) — functional proveedor picker (.NET search endpoint + SPA service + picker dialog + detalle-page wiring). **Accepted `size:exception`** (whole slice as ONE commit, ~575 authored lines, maintainer-approved per apply prompt). All suites green, `dotnet build SmartNet.sln` clean, SPA lint clean, prod build no budget warning. Not pushed, no PR.
- Next: Phase 7 (verification).

## Phase 8 — Functional proveedor picker (PR8, dep PR4) — DONE (15/15)

**Branch**: `pr8/item-18-proveedor-picker` off `pr6/item-18-asientorespuesta-base-igv-projection`. Strict TDD, RED→GREEN per task. **Accepted `size:exception`** — the whole picker slice ships as ONE commit (~575 authored lines); the three-way sub-split (8a/8b/8c) recommended in tasks.md was overridden by the ratified user decision to deliver it together.

### Ratified user decisions honored
- **P00000 ("Varios") EXCLUDED from search results** — `WHERE ... AND codpro <> 'P00000'` in `SqlProveedorRepository.BuscarAsync`; asserted by both the infra test and the API contract test. OPEN QUESTION 1 resolved as "excluded".
- **`dbo.Proveedor(proveedor)` name index OUT OF SCOPE** (ADR 0003 `dbo.*` object) — `LIKE` over ~6600 rows accepted, documented at the `TamanoPagina` const and in the spec.

| Task | Status | Notes |
|---|---|---|
| 8.1 GREEN csproj refs | [x] | `SmartNet.Catalogos.Core` + `.Infrastructure` `ProjectReference` added to `SmartNet.Api.csproj` (same Core/Infra pair shape as tipos-de-cambio; NEVER `SmartNet.Db.Runner` — `NoRunnerReferenceGuardTests` still green). |
| 8.2 RED infra test | [x] | `SqlProveedorRepositoryTests.cs` +9 (incl `[Theory]` x3): name fragment ordered by `proveedor`, RUC match, P00000 excluded, paging + `HayMas`, page past end, blank/short → empty, no-match. RED = compile CS1061 `BuscarAsync` / CS0117 `TamanoPagina` missing. |
| 8.3 GREEN repo search | [x] | `IProveedorRepository.BuscarAsync(consulta, pagina, ct) → BusquedaProveedores(Resultados, HayMas)` (new record in Core). `SqlProveedorRepository`: `const TamanoPagina = 20`, `LongitudMinima = 2`; trims `q`, returns empty (no query) when `< 2`; `WHERE (proveedor LIKE @patron OR rucpro LIKE @patron) AND codpro <> 'P00000' ORDER BY proveedor OFFSET @salto ROWS FETCH NEXT @tamano ROWS ONLY` fetching `TamanoPagina + 1` to derive `HayMas` without a COUNT round-trip. SELECT-only — `NoWriteToDboStructuralTests` still green (no INSERT/UPDATE/DELETE token in the file). |
| 8.4 RED API contract test | [x] | `CatalogoEndpointsTests.cs` (7 tests incl `[Theory]` x3, real SQL via `SmartNetApiFactory`/`SesionEndpointsTestBase`, dbo.Proveedor seeded via raw `Db.ExecuteNonQueryAsync`): name fragment ordered by `nombre` w/ `codigo`/`nombre`/`ruc`, RUC match, 2nd page + past-end + `hayMas`, missing/short `q` → 200 empty, no-match, P00000 excluded, 401 unauthenticated. RED = 404 (route absent). |
| 8.5 GREEN endpoint + Program.cs | [x] | `CatalogoEndpoints.cs` — `GET /api/catalogos/proveedores?q=&pagina=` `.RequireAuthorization()` → `{ resultados: { codigo, nombre, ruc }[], hayMas }` (`ProveedorResultado` / `BusquedaProveedoresRespuesta` internal records). `Program.cs`: `using SmartNet.Catalogos.Core/.Infrastructure`, lazy Singleton `IProveedorRepository` → `SqlProveedorRepository` (same pattern as every other repo), `app.MapCatalogoEndpoints()`. |
| 8.6 REFACTOR | [x] | Confirmed: no `fact.*` access, no writes, no versioned SQL, no new grant (`008_usuarios_y_permisos.sql:149` already `GRANT SELECT ON dbo.Proveedor`). `PermissionSufficiencyTests` (catalogos infra) still green. |
| 8.7 RED service spec | [x] | `catalogos/data-access/proveedor.service.spec.ts` (3 tests, `HttpTestingController`): debounced rapid input → exactly one `GET /api/catalogos/proveedores?q=ACME&pagina=1`, readonly signal parsed; `masResultados()` → `pagina=2` appended; blank/short term → no request, results cleared. RED = module `./proveedor.service` absent. |
| 8.8 GREEN service + model | [x] | `proveedor.model.ts` (`Proveedor { codigo, nombre, ruc }`, `BusquedaProveedoresRespuesta`). `proveedor.service.ts` — `@Injectable({ providedIn: 'root' })`, private `signal` + `asReadonly()` (`resultados`/`hayMas`/`buscando`), `firstValueFrom(http.get(...))`, `setTimeout` debounce (`debounceMs = 250`, mutable so specs shrink it — no fake timers), `LONGITUD_MINIMA = 2`, page-1 replaces / page-N appends. No state library (ADR 0009). |
| 8.9 RED picker spec | [x] | `detalle/ui/picker-proveedor/picker-proveedor.spec.ts` (7 tests, `HttpTestingController`): `<dialog role="dialog" aria-modal="true" aria-label>` open, debounced input, rows show `nombre`/`codigo`/`ruc`, row click emits `{ codigo, ruc }` + closes, ArrowDown moves `.picker-proveedor__fila--activa` + Enter selects, Escape closes w/o emit + emits `cerrar`, Tab keeps focus inside dialog, only the search GET (never PATCH). RED = component absent. |
| 8.10 GREEN picker dialog | [x] | `picker-proveedor.{ts,html,css}` — presentational, native `<dialog>` toggled via `.open` (jsdom-safe, same as `confirmar-reproceso`). Consumes `ProveedorService`; `seleccionar` output `{ codigo, ruc }`, `cerrar` output; `indiceActivo` signal for keyboard nav; `onKeydown` handles Arrow/Enter/Escape/Tab (simple focus trap over `input,button` focusables); `queueMicrotask` focus on `abrir()`. CSS layout-only: `--radio-modal`, `--sombra-prominente`, `--borde-sutil`, `--fondo-superficie`, `--accento-suave`, `--fs-*`, `--space-*` — **zero new token**. |
| 8.11 REFACTOR budget/tokens | [x] | `npx ng build --configuration production` → no budget warning (`picker-proveedor.css` ~1.4 kB < 4 kB). All values `var(--token)`; `contraste.spec.ts` / `paleta.spec.ts` untouched and green (34 files / 296). |
| 8.12 RED detalle-page wiring spec | [x] | `detalle-page.spec.ts` +3 (`By.directive`): `factura-form.buscarProveedor.emit()` → picker `<dialog>.open === true`; `picker.seleccionar.emit({codigo,ruc})` → `borradorFactura()` === `{ proveedorCodigo, rucProveedor }`, `expectNone(PATCH)`; picked value only PATCHed on `guardarAvance()`. RED = `PickerProveedor` not imported / `onProveedorSeleccionado` absent. |
| 8.13 RED factura-form spec | [x] | `factura-form.spec.ts` +1: `buscarProveedor` still emits `undefined` (payload-less, no new save contract) and the form renders no `<dialog>` of its own. |
| 8.14 GREEN wiring | [x] | `detalle-page.ts`: import + `imports:[... PickerProveedor ...]`, `onProveedorSeleccionado({codigo,ruc})` → `onCambiosFactura({ proveedorCodigo, rucProveedor? })` (rucProveedor included only when non-null). `detalle-page.html`: `(buscarProveedor)="picker.abrir()"` on `<app-factura-form>` + `<app-picker-proveedor #picker (seleccionar)="onProveedorSeleccionado($event)" />`. Existing draft path only — no new save contract, no direct PATCH. |
| 8.15 REFACTOR | [x] | Full `npx ng test --no-watch` → 34 files / **296 passed** (was 282 at PR6; +14). SPA lint clean. `dotnet build SmartNet.sln` → 0 warn / 0 err. `dotnet test SmartNet.Api.Tests` → **163 passed**. `dotnet test SmartNet.Catalogos.Infrastructure.Tests` → **66 passed**. `dotnet test SmartNet.Catalogos.Core.Tests` → **32 passed**. |

### Phase 8 TDD Cycle Evidence

| Task | Test File | Layer | Safety Net | RED | GREEN | TRIANGULATE | REFACTOR |
|---|---|---|---|---|---|---|---|
| 8.2/8.3 | `SqlProveedorRepositoryTests.cs` | Integration (real SQL) | ✅ 3/3 prior | ✅ compile CS1061/CS0117 | ✅ 12/12 | ✅ 9 cases (name/ruc/P00000/paging/past-end/blank×3/no-match) | ✅ `TamanoPagina`/`LongitudMinima` consts, +1 fetch for `HayMas` |
| 8.4/8.5 | `CatalogoEndpointsTests.cs` | Integration (real SQL + HTTP) | N/A (new) | ✅ 404 route absent | ✅ 7/7 | ✅ 9 cases incl 401 + P00000 + past-end | ✅ thin route, records mirror C#→camelCase |
| 8.7/8.8 | `proveedor.service.spec.ts` | Unit (`HttpTestingController`) | N/A (new) | ✅ module absent | ✅ 3/3 | ✅ debounce-collapse / append / short-term-clear | ✅ `debounceMs` mutable, `limpiar()` extracted |
| 8.9/8.10 | `picker-proveedor.spec.ts` | Component (jsdom + `HttpTestingController`) | N/A (new) | ✅ component absent | ✅ 7/7 | ✅ click-select / keyboard-select / Escape / Tab-trap / no-PATCH | ✅ `focusables()` + `atraparFoco()` extracted; token-only CSS |
| 8.12/8.14 | `detalle-page.spec.ts` | Component (jsdom) | ✅ prior detalle-page suite | ✅ import/handler absent | ✅ 3/3 | ✅ open / draft-not-server / persist-on-guardar | ➖ reuses `onCambiosFactura` verbatim |
| 8.13 | `factura-form.spec.ts` | Component (jsdom) | ✅ 25/25 prior | ✅ (guard test — asserts unchanged contract) | ✅ pass | ➖ single | ➖ none |

### Phase 8 Work Unit Evidence

| Evidence | Value |
|---|---|
| Focused test command + result | `.NET`: `dotnet test SmartNet.Catalogos.Infrastructure.Tests` → **66 passed**; `dotnet test SmartNet.Api.Tests` → **163 passed** (+7 Catalogo); `dotnet test SmartNet.Catalogos.Core.Tests` → **32 passed**. `SPA`: `npx ng test --no-watch --include='**/proveedor.service.spec.ts' --include='**/picker-proveedor.spec.ts' --include='**/detalle-page.spec.ts' --include='**/factura-form.spec.ts'` → **4 files / 63 passed**. Full `npx ng test --no-watch` → **34 files / 296 passed**. `npm run lint` clean. `dotnet build SmartNet.sln` → 0/0. `npx ng build --configuration production` → no budget warning. |
| Runtime harness | Real SQL Server via `SmartNetApiFactory` / `SmartNet.Db.TestBootstrap` — `CatalogoEndpointsTests` seed `dbo.Proveedor` rows, hit `GET /api/catalogos/proveedores?q=&pagina=` authenticated, assert ordering / paging / `hayMas` / P00000-exclusion / 401 round-trip against a migrated throwaway `fact_test_<id>` DB. `SqlProveedorRepositoryTests` exercise `BuscarAsync` directly against the same. |
| Rollback boundary | Revert: `SmartNet.Api.csproj` (2 `ProjectReference`), `Program.cs` (2 usings + 1 DI line + 1 `Map` line), delete `CatalogoEndpoints.cs`; revert `IProveedorRepository.cs` (method + `BusquedaProveedores` record) and `SqlProveedorRepository.cs` (`BuscarAsync` + consts); revert test additions in `SqlProveedorRepositoryTests.cs`, delete `CatalogoEndpointsTests.cs`. SPA: delete `src/app/catalogos/`, delete `src/app/detalle/ui/picker-proveedor/`; revert `detalle-page.{ts,html}` (import + 1 array entry + `onProveedorSeleccionado` + 2 template lines), revert the +1 test in `factura-form.spec.ts` and +3 in `detalle-page.spec.ts`. No PR1–PR6 file touched except `detalle-page.*` (additive) and the two spec files. Reverting the SPA does not break the API and vice versa. |

### Phase 8 risks / deviations
- **`size:exception` (~575 authored lines > 400 budget)** — accepted by the maintainer per the apply prompt; the tasks.md three-way stacked sub-split (PR8a/8b/8c) was deliberately overridden by the ratified "one commit" decision. ~230 of the ~575 lines are new test code.
- **`LIKE '%term%'` wildcards not escaped** — a `q` containing `%` / `_` / `[` is treated as a pattern. Accepted for a human-typed picker search over a ~6600-row `dbo.*` catalog (ADR 0003, no index); revisit only if it becomes a problem.
- **Min query length is 2** (`LongitudMinima` / `LONGITUD_MINIMA`) — the spec allows "at least 1"; 2 chosen to keep the unindexed `LIKE` scan meaningful. Consistent server + client.
- **`ProveedorService` debounce uses `setTimeout`, not RxJS `debounceTime`** — the repo has no `debounceTime` precedent and its specs never use fake timers; a mutable `debounceMs` lets specs use a 5 ms real-timer window. Same "emit only on settle" behavior.
- **`rucProveedor` is only pushed into the draft when the picked proveedor has one** — matches spec "(and `rucProveedor` when applicable)"; a `null` ruc would mean "untouched" server-side anyway, but omitting it keeps the draft payload honest.
- **`P00000` exclusion is a hard `WHERE` filter**, not an `esGenerico` flag — OPEN QUESTION 1 resolved per the ratified user decision; if product later wants "shown but marked" the repo `WHERE` clause and the two exclusion tests are the single change point.
