...
needs a search endpoint. No such endpoint existed before item #18 (investigation
...
- `LIKE` wildcard characters in `q` are not escaped (carried forward as a follow-up SUGGESTION from verify; no security impact, the query is parameterised).
