...
Aceptado. Revisión 2. Corrige la lista de opciones de certificado, que incluía una imposible
(revisión adversarial v2, S1).

Decisión nueva en la revisión 1: el diseño anterior tenía diez ADRs, tres artefactos desplegables y
**cero decisiones sobre dónde y cómo se ejecutan**.
...
**Pendiente:** el origen del certificado y quién lo renueva. Las opciones reales son **dos**, no
tres:
...
> **Corrección (revisión adversarial v2, S1).** La lista anterior incluía Let's Encrypt como tercera
> opción sobre el host `facturas.empresa.local`, y **no es viable**: Let's Encrypt exige un dominio
> público validable, y `.local` está reservado para mDNS por RFC 6762, de modo que ni Let's Encrypt
> ni una autoridad comercial pueden emitir para él. La decisión seguía pendiente, pero arrancaba
> desde una opción falsa.
...
- **Pendiente:** origen y renovación del certificado TLS. La elección real es entre autoridad interna
  y dominio público con DNS interno; Let's Encrypt sobre `.local` no es una opción (S1).
