...
- Lista de facturas validadas en formato **cabecera**: número de comprobante, origen del libro (por defecto 02 Compras), proveedor, glosa, fecha contable, tipo de cambio, base imponible, IGV, neto y estado del asiento.
...
- Estado de conexión con Gmail, Drive y Google Sheets (conectado / con error). El dato lo entrega el servidor: última ejecución, último éxito, último error y fallos seguidos por integración. La píldora **se deriva** de esos campos, no se almacena como estado.
- El **worker** aparece en la misma lista. Su fila es el latido que permite saber que sigue vivo.
